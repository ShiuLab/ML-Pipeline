...
def main():
		...
		if args.alg.lower() == 'lr':
			...
			if args.alg.lower() == 'rf':	
				params2use, param_names = RF.fun.RegGridSearch(df, args.save, \
					args.gs_score, args.cv_num, args.n_jobs, args.gs_reps, \
					args.gs_type, args.gs_full)
	...
	for j in range(0, args.n):
		...
		if args.alg.lower() == "rf":
			reg = RF.fun.DefineReg_RandomForest(args.n_estimators,
		...
		if args.test != '':
			reg, result, cv_pred, importance, result_test = RF.fun.Run_Regression_Model(
				df, reg, args.cv_num, df_unknowns, test_df, args.cv_sets, j)
			results_test.append(result_test)
		else:
			reg, result, cv_pred, importance = RF.fun.Run_Regression_Model(
				df, reg, args.cv_num, df_unknowns, test_df, args.cv_sets, j)
