cat("
args:
inp1 = Input file (col1 = id ,col2 = type, col3 = value), header required
inp2 = threshold
...
for(typ in types){
	...
	hist_obj <- hist(as.numeric(type_values),breaks=c(seq(from=0,to=1,by=0.05)),main=typ,xlab="ML_score",ylab="count",col=color_vector,border=FALSE)
