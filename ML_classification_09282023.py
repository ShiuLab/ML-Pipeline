...
def main():
	...
	if args.gs.lower() in ['t', 'true']:
		...
		if args.alg.lower() == 'rf':
			params2use, balanced_ids, param_names = RF.fun.GridSearch(df,
			args.save, classes, min_size, args.gs_score, args.n, args.cv_num,
	...
	for j in range(len(balanced_ids)):
		...
		if args.alg.lower() == "rf":
			parameters_used = [args.n_estimators, args.max_depth,
				args.max_features]
			clf = RF.fun.DefineClf_RandomForest(args.n_estimators,
		...
		if args.test != '':
			...
			clf, result, current_scores, result_test = \
				RF.fun.Run_Classification_Model(df1, clf, args.cv_num,
					df_notSel, apply_unk, df_unknowns, test_df, classes,
					args.pos, NEG, j, args.threshold_test)
			results_test.append(result_test)
		else:
			# also return the fitted clf to be saved, Peipei Wang, 12/06/2021
			clf, result, current_scores = RF.fun.Run_Classification_Model(df1,
				clf, args.cv_num, df_notSel, apply_unk, df_unknowns, test_df,
				classes, args.pos, NEG, j, args.threshold_test)
	...
	conf_matrices = pd.DataFrame(columns=np.insert(arr=CLASSES,
	...
	AucPRc_array = []
	accuracies = []
	f1_array = np.array([np.insert(arr=classes.astype(str),
		obj=0, values='M')])
	...
	if args.test!='':
		...
		AucPRc_test_array = []
		accuracies_test = []
		f1_array_test = np.array([np.insert(arr=classes.astype(str),
			obj=0, values='M')])
